...
export function PixelRevealMarkHollowDot() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      for (const tile of tiles) {
        ...
        const sx = Math.round(tile.x) + 0.5;
        const sy = Math.round(tile.y) + 0.5;
        ctx.strokeStyle = tile.c;
        ctx.strokeRect(sx, sy, tile.size, tile.size);
        const dotSize = tile.size * 0.26;
        ctx.fillStyle = tile.c;
        ctx.fillRect(
          tile.x + (tile.size - dotSize) / 2,
          tile.y + (tile.size - dotSize) / 2,
          dotSize,
          dotSize
        );
      }
